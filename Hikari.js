...
var ara = fs.readFileSync('./media/ara.mp3')
...
let Qrisjpg = fs.readFileSync("./media/Qris.jpg")
...
sourceUrl: "https://youtu.be/KAglIQ3bUls"
               ...
               Hikari.relayWAMessage(prep)
               Hikari.sendMessage(from, ara, MessageType.audio, { quoted: freply, mimetype: 'audio/mp4', ptt: true })
               break
...
case 'modesimi':
        ...
        buttons = [{buttonId: `${prefix}infoig`,buttonText:{displayText: ` ©WhatsApp bot`},type:1}]
              ...
              contentText:`©${botName}`,buttons,headerType:4}
...
anu = await fetchJson(`https://leyscoders-api.herokuapp.com/api/ppcouple?apikey=IkyOgiwara`)
            ...
            buttons = [{buttonId: `${prefix}infoig`,buttonText:{displayText: `©${botName}`},type:1}]
...
`
   ...
   case 'itemmenu':
   case 'storemenu':
   wibu =` STORE/ITEM MENU
●${prefix}additem NamaItem|ListHarga
●${prefix}listitem 
●${prefix}cekitem NamaItem
●${prefix}changeitem NamaItem|NewList
 ●${prefix}dellitem NamaItem 
`
               sendButMessage(from, wibu, `Save Nomor Bot☜☆☞`, [
          {
            buttonId: `${prefix}menu`,
...
Ket : Ketik /resetgame , Untuk Mereset Permainan Yg Ada Di Grup!`, text, {contextInfo: {mentionedJid: [player2]}})
   ...
   sendButMessage(from, titid, `Klik Untuk Ke Game Selanjutnya`, [
       ...
       case 'tebaklirik':
              ...
              case 'command':
               list = []
               listmenu = [`anonymous`,`adventuremenu`,`allmenu`,`jadibotmenu`,`itemmenu`,`groupmenu`,`photoxy`,`ephoto`,`randomimage`,`wibu2`,`storymenu`,`porno`,`randomtext`,`islammenu`,`wibumenu`,`stickermenu`,`ownermenu`,`gamemenu`,`funmenu`,`downloadmenu`,`infomenu`,`othermenu`,`owner`,`hikarigroup`,`sewabot`]
               listmenuu = [`Anonymous Chat`,`Role Player Menu`,`All Menu`,`Jadi Bot Menu`,`Item Menu Store`,`Menu Group`,`Photo Oky`,`Ephoto Menu`,`Random Image`,`Nsfw Anime`,`Movie&Story`,`18+ Menu`,`RandomText`,`Islam Menu`,`Wibu Menu`,`Sticker Menu`,`Owner Command`,`Game Menu`,`For Fun Menu`,`Downloader`,`Info Menu`,`Menu Lainnya`,`OwnerBot`,`Official Group`,`Rent this Bot`]
               nombor = 1
               startnum = 0
...
└─❏
`
 let buttouns = [{ buttonId: `${prefix}payment`, buttonText: { displayText: 'PAYMENT' }, type: 1 } ]
               imageMsg = (await Hikari.prepareMessageMedia(fs.readFileSync(`./media/Qris.jpg`), 'imageMessage', { thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})).imageMessage

               buttonsMessage = {
               contentText: `${teksnya}`,
               footerText: `Bot By @${setting.ownerName}`, imageMessage: imageMsg,
               buttons: buttouns,
               headerType: 4
}
               prep = await Hikari.prepareMessageFromContent(from,{buttonsMessage},{contextInfo: {mentionedJid: [sode], quoted: freply}})
...
NOTE* JANGAN LUPA CONFIRM BUKTI TRANSFER KEPADA OWNER ATAU KLIK TOMBOL CONFIRM!`

              


    Hikari.sendMessage(from, { contentText: `${menu}`, footerText: `${setting.ownerName}`, buttons: [{ buttonId: `${prefix}bukti`, buttonText: { displayText: 'CONFIRM BUKTI TF' }, type: 1 },{ buttonId: `${prefix}payment`, buttonText: { displayText: 'GANTI PAYMENT' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: Hikariown, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
...
Jika sudah tf sesuai nominal silahkan CONFIRM BUKTI ke owner ku ya!
...
NOTE* JANGAN LUPA CONFIRM BUKTI TRANSFER KEPADA OWNER ATAU KLIK TOMBOL CONFIRM!`
    ...
    let buttoins = [{ buttonId: `${prefix}bukti`, buttonText: { displayText: 'CONFIRM BUKTI TF' }, type: 1 },{ buttonId: `${prefix}payment`, buttonText: { displayText: 'GANTI PAYMENT' }, type: 1 } ]
               imageMsg = (await Hikari.prepareMessageMedia(fs.readFileSync(`./media/Qris.jpg`), 'imageMessage', { thumbnail:Bfake, contextInfo:{forwardingScore: 989, isForwarded: true}})).imageMessage
      ...
      case 'payment':
               list = []
               listmenu = [`qris`,`ovo`,`pulsa`]
               listmenuu = [`BAYAR DENGAN QRIS`,`BAYAR DENGAN OVO`,`BAYAR DENGAN PULSA`,`INFO BISNIS`,]
               nombor = 1
               startnum = 0
               for (let x of listmenu) {
...
break
       ...
       if (!isOwner) return 
        ...
        case 'autoreject':
       if (!isOwner) return 
        if (!args.length) return reply(`Auto Reject\n\n- true\n- multi\n\n- false\n\nExample : ${prefix}prefix true`)
        if (args[0] === "true") {
          setting.callBlock = "true"
          fs.writeFileSync('./setting.json', JSON.stringify(setting))
          reply(`Auto Reject Aktif`)
        } else if (q === "false") {
          setting.callBlock = "false"
          fs.writeFileSync('./setting.json', JSON.stringify(setting))
          reply(`Auto Reject Mati`)
        } else {
          reply("pilih true, atau false")
        }
        break
        case "setprefix":
        if (!isOwner) return 
...
//CASE ITEM PRODUCT BY MANIK NO COPAS
case 'additem':{
...
const rpg = `*${getReason}* `
Hikari.sendMessage(from, { contentText: `${rpg}`, footerText: `${setting.ownerName}`, buttons: [{ buttonId: `${prefix}bukti`, buttonText: { displayText: 'CONFIRM BUKTI TF' }, type: 1 },{ buttonId: `${prefix}payment`, buttonText: { displayText: 'PAYMENT' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: Hikariown, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
}
case 'listitem':{
let ane = []
...
case 'addjudullist':
          if (args.length === 0) return reply(`Buat list dengan judul\n\nContoh : ${prefix}addjudullist | <judul list>`);
          if (judullist.length > 0) return reply(`Mohon untuk reset list terlebih dahulu dengan command ${prefix}resetlist`);
          const isijudullist = arg.split(`|`)[1];
          const judulin = judullist.push(isijudullist);
          if (judulin) return reply(`List sudah ditambahkan, untuk menambahkan isi list menggunakan command ${prefix}addlist | <isi list>`);
          break;
        
        case 'addlist':
          if (args.length === 0) return reply(`Tambah daftar List dengan isi\n\nContoh : ${prefix}addlist | <ini list>`);
          if (judullist.length === 0) return reply(`Mohon untuk membuat judul List terlebih dahulu dengan command ${prefix}addjudullist`);
        ...
        case 'hapuslist':
          if (args.length === 0) return reply(`Hapus item pada List dengan nomor item\n\nContoh : ${prefix}hapuslist 1`);
          if (daftarlist.length === 0) return reply(`Tambah daftar List dengan isi\n\nContoh : ${prefix}addlist | <ini list>`);
          if (judullist.length === 0) return reply(`Mohon untuk membuat judul List terlebih dahulu dengan command ${prefix}addjudullist`);
          var i = args[0];
          i--;
        ...
        case 'resetlist':
          ...
          if (daftarlist.length === 0 && judullist.length === 0) return reply(`List sudah di reset !`);
          break;
    case 'getses':
          if (!isOwner) return await reply('Owner Only')
          const ses = await Hikari.getSnapshot()
           sendFile(from, ses, 'session.png', 'done')
      break
